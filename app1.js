...
async function authFetch(path,body){
  const res=await fetchWithTimeout(`${SUPABASE_URL}/auth/v1/${path}`,{method:'POST',headers:{'apikey':SUPABASE_KEY,'Content-Type':'application/json'},body:JSON.stringify(body||{})});
...
async function refreshAuthSession(remember=true){
  ...
  authSession=normalizeSession(data);currentUser=authSession.user;persistSession(authSession,remember);return authSession;
}

async function ensureAccessToken(){
  if(!authSession)throw new Error('Faça login novamente.');
  if(Number(authSession.expires_at||0)<Math.floor(Date.now()/1000)+60){const remembered=!!localStorage.getItem(SESSION_KEY);await refreshAuthSession(remembered)}
...
async function sbRest(path,options={},retry=true){
  ...
  if(res.status===401 && retry && authSession?.refresh_token){const remembered=!!localStorage.getItem(SESSION_KEY);await refreshAuthSession(remembered);return sbRest(path,options,false)}
  ...
  if(!res.ok){
    ...
    const msg=(data&&typeof data==='object'&&(data.message||data.details||data.hint))||`Erro ${res.status} ao acessar o banco.`;throw new Error(msg);
  }
  return data;
}
...
function cloudToRecord(c){
  ...
  const mappedChannels=channels.map(name=>{const x=channelMap.get(name)||{};return {name,q:Number(x.order_count||0),v:Number(x.amount||0)}});
  ...
  const sales=Number(c.total_sales||0),expense=Number(c.total_expenses||0),deliveryCash=Number(c.delivery_cash_sales||0);
  ...
  const record={_id:c.id,status:c.status||'closed',registerName:c.register_name||'Caixa Principal',shiftName:c.shift_name||'Dia',date:c.business_date,resp:c.responsible_name||'',opening:Number(c.opening_balance||0),cash:Number(c.cash_sales||0),deliveryCash,cardOut:Number(c.store_card_sales||0),onlinePayment:Number(c.pix_app_sales||0),deliveryCard:Number(c.delivery_card_sales||0),cashOut:Number(c.cash_withdrawn_for_expenses||0),cashCountVerified:!!c.cash_count_verified,countedCash:Number(c.counted_cash||0),expectedCash:Number(c.expected_cash||0),cashDifference:Number(c.cash_difference||0),paymentTotal,paymentDifference:paymentTotal-sales,summaryTotal:paymentTotal,sales,orders:mappedChannels.reduce((a,x)=>a+x.q,0),expense,result:Number(c.result||0),balance:Number(c.result||0),channels:mappedChannels,online:{anotaQtd:Number(anota.order_count||0),anotaVal:Number(anota.amount||0),aiqQtd:Number(aiq.order_count||0),aiqVal:Number(aiq.amount||0),orders:Number(anota.order_count||0)+Number(aiq.order_count||0),value:Number(anota.amount||0)+Number(aiq.amount||0)},expenses:(c.expenses||[]).map(x=>({d:x.description||'',val:Number(x.amount||0)})),obs:c.observations||'',breads:{idealStart:Number(ideal.opening_stock||0),idealProd:Number(ideal.production||0),idealOut:Number(ideal.out_qty||0),idealFinal:Number(ideal.closing_stock||0),gourmetStart:Number(gourmet.opening_stock||0),gourmetProd:Number(gourmet.production||0),gourmetOut:Number(gourmet.out_qty||0),gourmetFinal:Number(gourmet.closing_stock||0)},savedAt:c.updated_at||c.created_at||''};
  return window.XBBusinessRules?.normalizeRecord?window.XBBusinessRules.normalizeRecord(record,{cashCountVerified:record.cashCountVerified}):record;
}
...
async function loadCloudData(){
  ...
  cloudLoadPromise=(async()=>{
    ...
    try{
      ...
      while(true){const rows=await sbRest(`cash_closings?select=${encodeURIComponent(select)}&order=business_date.asc&limit=${pageSize}&offset=${offset}`);const batch=Array.isArray(rows)?rows:[];all.push(...batch);if(batch.length<pageSize)break;offset+=pageSize}
      cloudData=all.map(cloudToRecord);lastSyncAt=new Date();setCloudStatus('● Banco na nuvem','online');updateSyncUi();return cloudData;
    }catch(err){setCloudStatus(navigator.onLine?'● Erro de sincronização':'● Sem internet','error');throw err}
...
async function loadProfile(){
  ...
  currentProfile=rows?.[0]||null;if(currentProfile&&currentProfile.active===false)throw new Error('Este usuário está desativado.');
  const email=currentUser.email||'Usuário';$('userName').textContent=currentProfile?.full_name||email.split('@')[0];const roleLabel={admin:'Administrador',manager:'Gerente',operator:'Operador'}[currentProfile?.role]||'Usuário';$('userRole').textContent=roleLabel;return currentProfile;
}

async function loginCloud(email,password,remember){
  const data=await authFetch('token?grant_type=password',{email,password});authSession=normalizeSession(data);if(!authSession?.access_token)throw new Error('O servidor não retornou uma sessão válida.');currentUser=authSession.user;await loadProfile();await loadCloudData();persistSession(authSession,remember);
}

async function saveRecordCloud(rec){
  const payload={...rec,status:rec.status||'closed'};
  return sbRest('rpc/save_cash_closing',{method:'POST',headers:{'Prefer':'return=representation'},body:JSON.stringify({p_record:payload})});
}

function refreshDailyReport(){
  ...
  if(!rec)return;
  const r=rec,expenses=Array.isArray(r.expenses)?r.expenses:[];
  const paymentTotal=Number(r.paymentTotal??((r.cash||0)+(r.deliveryCash||0)+(r.cardOut||0)+(r.onlinePayment||0)+(r.deliveryCard||0)));
  ...
  $('dailyFinancialRows').innerHTML=[
    reportValueRow('Saldo inicial',r.opening),
    reportValueRow('Vendas em dinheiro',r.cash),
    reportValueRow('Dinheiro — entregas',r.deliveryCash||0),
    ...
    reportValueRow('Dinheiro contado',cashCountVerified?r.countedCash:0,cashCountVerified?'Informado':'Não informado'),
    reportValueRow('Diferença do caixa',cashCountVerified?cashDiff:0,cashCountVerified?(Math.abs(cashDiff)<.005?'Conferido':cashDiff>0?'Sobra':'Falta'):'Opcional'),
    reportValueRow('Resultado do dia',r.result)
  ].join('');
  if(!cashCountVerified){
    document.querySelectorAll('#dailyFinancialRows .data-row').forEach(row=>{
      ...
      if(label==='Diferença do caixa'&&value){value.textContent='—';value.classList.remove('money','positive','negative');}
    });
  }
...
function refreshMonthly(){
  ...
  ];$('monthlyPaymentsTable').innerHTML=payments.map(([l,v])=>`<tr><td>${l}</td><td class="${Number(v)<0?'negative':''}"><b>${br(v)}</b></td></tr>`).join('');
  ...
  $('monthTable').innerHTML=month.length?month.map(r=>`<tr><td>${reportDateBr(r.date)}</td><td>${escapeHtml(r.resp||'—')}</td><td>${br(r.sales)}</td><td>${br(r.cash)}</td><td>${br(r.deliveryCash||0)}</td><td>${br(r.cardOut)}</td><td>${br(r.deliveryCard)}</td><td>${br(r.onlinePayment)}</td><td>${br(r.expense)}</td><td>${br(r.cashOut)}</td><td class="${r.result<0?'negative':'positive'}"><b>${br(r.result)}</b></td><td>${r.orders||0}</td><td class="${r.cashCountVerified?(r.cashDifference<0?'negative':r.cashDifference>0?'positive':''):''}">${r.cashCountVerified?br(r.cashDifference||0):'—'}</td><td>${escapeHtml(r.obs||'')}</td></tr>`).join(''):`<tr><td colspan="14"><div class="empty">Nenhum fechamento registrado neste mês.</div></td></tr>`;
...
/* Fallback simples: backup-protection.js substitui esta rotina pela exportação assinada SHA-256. */
function exportJSON(){const data={version:String(window.XB_APP_VERSION||'4.18.3'),exportedAt:new Date().toISOString(),records:load()};download(`xburguer-backup-${isoToday()}.json`,JSON.stringify(data,null,2),'application/json');const now=new Date().toLocaleString('pt-BR');localStorage.setItem(BACKUP_KEY,now);refreshBackup();toast('Backup JSON exportado.')}

function exportCSV(){const rows=[['Data','Responsável','Vendas','Despesas','Dinheiro retirado p/ despesas','Resultado','Pedidos','Saldo inicial','Dinheiro caixa','Dinheiro entregas','Cartão loja','Cartão entregas','Pix/App','Diferença Caixa','Pão Ideal Est. inicial','Pão Ideal Produção','Pão Gourmet Est. inicial','Pão Gourmet Produção','Observações'],...load().map(normalize).map(r=>[r.date,r.resp,r.sales,r.expense,r.cashOut||0,r.result,r.orders,r.opening||0,r.cash||0,r.deliveryCash||0,r.cardOut||0,r.deliveryCard||0,r.onlinePayment||0,r.cashCountVerified?r.cashDifference:'',r.breads?.idealStart||0,r.breads?.idealProd||0,r.breads?.gourmetStart||0,r.breads?.gourmetProd||0,r.obs])];const csv='\ufeff'+rows.map(row=>row.map(v=>'"'+String(v??'').replace(/"/g,'""')+'"').join(';')).join('\n');download(`xburguer-caixa-${isoToday()}.csv`,csv,'text/csv;charset=utf-8');toast('Planilha CSV exportada.')}

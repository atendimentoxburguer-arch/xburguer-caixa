alter table public.cash_closings
  add column if not exists delivery_cash_sales numeric(12,2) not null default 0;
...
begin
  ...
  if v_ideal_final > v_ideal_start then
    raise exception 'Pão Ideal closing stock cannot exceed opening stock';
  end if;
  if v_gourmet_final > v_gourmet_start then
    raise exception 'Pão Gourmet closing stock cannot exceed opening stock';
  end if;

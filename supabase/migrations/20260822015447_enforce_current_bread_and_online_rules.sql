...
declare
  ...
  v_ideal_prod integer := coalesce((p_record#>>'{breads,idealProd}')::integer,0);
  v_gourmet_start integer := coalesce((p_record#>>'{breads,gourmetStart}')::integer,0);
  v_gourmet_prod integer := coalesce((p_record#>>'{breads,gourmetProd}')::integer,0);
...
begin
  ...
  if least(v_ideal_start,v_ideal_prod,v_gourmet_start,v_gourmet_prod) < 0 then
    raise exception 'bread quantities cannot be negative';
  end if;
  if v_ideal_prod > v_ideal_start then
    raise exception 'Pão Ideal production cannot exceed opening stock';
  end if;
  if v_gourmet_prod > v_gourmet_start then
    raise exception 'Pão Gourmet production cannot exceed opening stock';
  end if;
  ...
  values
    (v_id,'Pão Ideal',v_ideal_start,v_ideal_prod,0,v_ideal_start-v_ideal_prod),
    (v_id,'Pão Gourmet',v_gourmet_start,v_gourmet_prod,0,v_gourmet_start-v_gourmet_prod);

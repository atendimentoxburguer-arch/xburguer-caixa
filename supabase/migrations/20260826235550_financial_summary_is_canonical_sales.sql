...
begin
  ...
  -- Canais são somente demonstrativos e nunca definem a venda financeira.
  for ch in select value from jsonb_array_elements(coalesce(p_record->'channels','[]'::jsonb)) loop
    ...
    v_channel_sales := v_channel_sales + coalesce((ch->>'v')::numeric,0);
  end loop;

  -- Venda oficial = Resumo Financeiro/Formas de pagamento.
  v_sales := round(v_cash + v_delivery_cash + v_store_card + v_pix_app + v_delivery_card, 2);
  ...
  -- Saldo inicial não é venda, porém é dinheiro físico disponível na gaveta.
  v_expected := v_opening + v_cash + v_delivery_cash - v_cash_out;
  ...
  v_result := round(v_sales - v_expenses, 2);
...
-- Corrige todos os fechamentos já salvos sem alterar os demonstrativos por canal.
select set_config('xburguer.save_rpc','on',true);
update public.cash_closings
set total_sales = round(
      coalesce(cash_sales,0)
    + coalesce(delivery_cash_sales,0)
    + coalesce(store_card_sales,0)
    + coalesce(pix_app_sales,0)
    + coalesce(delivery_card_sales,0), 2),
    result = round(
      coalesce(cash_sales,0)
    + coalesce(delivery_cash_sales,0)
    + coalesce(store_card_sales,0)
    + coalesce(pix_app_sales,0)
    + coalesce(delivery_card_sales,0)
    - coalesce(total_expenses,0), 2)
where total_sales is distinct from round(
      coalesce(cash_sales,0)
    + coalesce(delivery_cash_sales,0)
    + coalesce(store_card_sales,0)
    + coalesce(pix_app_sales,0)
    + coalesce(delivery_card_sales,0), 2)
   or result is distinct from round(
      coalesce(cash_sales,0)
    + coalesce(delivery_cash_sales,0)
    + coalesce(store_card_sales,0)
    + coalesce(pix_app_sales,0)

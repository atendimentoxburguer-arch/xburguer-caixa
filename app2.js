...
function migrateLegacyDraft(){
  try{
    ...
    if(legacy?.date&&!localStorage.getItem(draftKey(legacy.date)))localStorage.setItem(draftKey(legacy.date),JSON.stringify(legacy));
    localStorage.removeItem(LEGACY_DRAFT_KEY);
  }catch{try{localStorage.removeItem(LEGACY_DRAFT_KEY)}catch{}}
}

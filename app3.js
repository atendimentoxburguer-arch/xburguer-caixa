function validateRecord(rec){
  ...
  const values=[rec.opening,rec.cash,rec.deliveryCash,rec.cardOut,rec.onlinePayment,rec.deliveryCard,rec.cashOut,rec.countedCash,...rec.channels.map(c=>c.v),rec.online.anotaVal,rec.online.aiqVal,...rec.expenses.map(e=>e.val)];
...
function buildSaveWarnings(rec){
  ...
  if(rec.cashCountVerified&&Math.abs(Number(rec.cashDifference||0))>=0.01){
    warnings.push('• A contagem física da gaveta tem diferença de '+br(rec.cashDifference)+'.');
  }
  return warnings;
}
...
async function deleteRecord(date){
  ...
  if(!r)return;
  const ok=await openConfirmModal({title:'Excluir fechamento',message:`Deseja excluir o fechamento de ${new Date(date+'T12:00:00').toLocaleDateString('pt-BR')}?`,note:'Uma cópia de recuperação protegida será mantida no banco.',confirmText:'Excluir',badge:'Exclusão'});
  if(!ok)return;
...
function drawChart(targetId,records){
  ...
  for(let d=1;d<=daysInMonth;d++){
    const ds=`${ym}-${String(d).padStart(2,'0')}`,r=month.find(x=>x.date===ds),s=Number(r?.sales||0),e=Number(r?.expense||0);
  ...
  $(targetId).innerHTML=`<div class="chart-wrap"><div class="chart-y">${steps.map(x=>`<span>${x>=1000?(x/1000).toFixed(1)+'k':Math.round(x)}</span>`).join('')}</div><div class="chart-area"><div class="chart-grid"><i></i><i></i><i></i><i></i><i></i></div><div class="bars">${bars}</div></div></div>`;
}

function refreshDashboard(){
  const ym=monthNow(),month=monthRecords(ym).map(normalize),sum=f=>month.reduce((a,r)=>a+Number(f(r)||0),0),sales=sum(r=>r.sales),exp=sum(r=>r.expense),orders=sum(r=>r.orders),days=month.filter(r=>r.sales||r.orders||r.expense).length,result=sales-exp;
  $('dashMonth').textContent=new Date(ym+'-01T12:00:00').toLocaleDateString('pt-BR',{month:'long',year:'numeric'});$('dSales').textContent=br(sales);$('dExp').textContent=br(exp);$('dRes').textContent=br(result);setTone($('dRes'),result);$('dResultHint').textContent=result>0?'Resultado positivo':result<0?'Resultado negativo':'Sem movimento';$('dTicket').textContent=br(orders?sales/orders:0);$('dDays').textContent=days+' dias com movimento';$('dOrders').textContent=orders+' pedidos';$('dCash').textContent=br(sum(r=>Number(r.cash||0)+Number(r.deliveryCash||0)));$('dCard').textContent=br(sum(r=>(r.cardOut||0)+(r.deliveryCard||0)));$('dPix').textContent=br(sum(r=>r.onlinePayment));drawChart('chart',month);
  const all=load().map(normalize),last=all.at(-1);
  if(last){$('lastSaveBadge').textContent='Último: '+new Date(last.date+'T12:00:00').toLocaleDateString('pt-BR');$('lastClosing').className='';$('lastClosing').innerHTML=`<div class="data-row"><span>Data</span><b>${new Date(last.date+'T12:00:00').toLocaleDateString('pt-BR')}</b><span></span></div><div class="data-row"><span>Responsável</span><b>${escapeHtml(last.resp||'—')}</b><span></span></div><div class="data-row"><span>Vendas</span><b class="money">${br(last.sales)}</b><span>${last.orders||0} pedidos</span></div><div class="data-row"><span>Resultado</span><b class="${last.result<0?'negative':'positive'}">${br(last.result)}</b><span></span></div>`}else{$('lastSaveBadge').textContent='Nenhum fechamento salvo';$('lastClosing').className='empty';$('lastClosing').textContent='Ainda não há fechamentos salvos.'}
}

function refreshHistory(){
  const ym=$('historyMonth').value,q=$('historySearch').value.trim().toLowerCase();
  let data=load().map(normalize).slice().reverse();
  if(ym)data=data.filter(r=>r.date.startsWith(ym));
  if(q)data=data.filter(r=>(r.resp||'').toLowerCase().includes(q)||(r.obs||'').toLowerCase().includes(q));
  $('historyCount').textContent=data.length+' registros';
  $('historyTable').innerHTML=data.length?data.map(r=>`<tr><td>${new Date(r.date+'T12:00:00').toLocaleDateString('pt-BR')}</td><td>${escapeHtml(r.resp||'—')}</td><td>${br(r.sales)}</td><td>${br(r.expense)}</td><td class="${r.result<0?'negative':'positive'}"><b>${br(r.result)}</b></td><td>${r.orders||0}</td><td class="${r.cashCountVerified?(r.cashDifference<0?'negative':r.cashDifference>0?'positive':''):''}">${r.cashCountVerified?br(r.cashDifference||0):'—'}</td><td>${escapeHtml(r.obs||'')}</td><td><div class="table-actions"><button class="link-btn" type="button" data-edit-date="${r.date}">Editar</button><button class="link-btn danger" type="button" data-delete-date="${r.date}">Excluir</button></div></td></tr>`).join(''):`<tr><td colspan="9"><div class="empty">Nenhum fechamento encontrado.</div></td></tr>`;
}

$('historyTable')?.addEventListener('click',event=>{
  const edit=event.target.closest('[data-edit-date]');
  if(edit){editRecord(edit.dataset.editDate);return;}
  const del=event.target.closest('[data-delete-date]');
  if(del)deleteRecord(del.dataset.deleteDate);
});
...
function reportValueRow(label,value,extra=''){return `<div class="data-row"><span>${escapeHtml(label)}</span><b class="${Number(value)<0?'negative':'money'}">${br(value)}</b><span>${escapeHtml(extra)}</span></div>`}
